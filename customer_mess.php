	...
	<div class="header">
		<div class="container">
			<div class="top-header">
				<div class="logo">
					<a href="customer_homepage.php"><img src="images/logo.png" class="img-responsive" alt="" /></a>
				</div>
				<div class="queries">
					...
					<div class="dotted-line">
						...
						<?php 
							...
							if(isset($_SESSION['customer_email'])) {
								...
								// check whether registered to mess or not 
								$dbhost = 'localhost';
								$dbuser = 'root'; 
								$dbpass = 'root'; 
								...
								if ($ret) {
									...
									if($row[0] == NULL) {
										echo "<h4>You haven't Registered for any mess.</h4><br>";
		...
		<div class="banner wow fadeInUp" data-wow-delay="0.4s" id="Home">
			...
			<div class="banner-info">
				<div class="banner-info-head text-center wow fadeInLeft" data-wow-delay="0.5s">
					<div class="line">
						<h2>Search For Mess</h2>
					</div>
				</div>
				<div class="form-list wow fadeInRight" data-wow-delay="0.5s">
					<form action="search_mess.php" method="GET" >
						  
						  <ul class="navmain">
							<li><span>Location Name</span>
							 <input type="text" class="text" placeholder="Enter Your Area" name="area">
							</li>
							
						  </ul>
						  <br>
							<h4><input class=srch type="submit" value="Search Mess"></h4>
					</form>
				</div>	
			</div>
		    </div>
									...
									<?php
										...
										echo "<h4>You have Registered for ";			
										$querry2 = "SELECT mess_name FROM mess where mess_id = $row[0]";
										...
										if ($ret2) {
											...
											$querry3 = "select * from todays_menu where mess_id=$row[0] and date='$curr_date'";
											$ret3 = mysql_query($querry3,$conn );
											
											if($ret3){
												$row3 = mysql_fetch_array($ret3, MYSQL_NUM);
												if($row3){
												$_SESSION['menu']=$row3[0];
												$_SESSION['mess']=$row[0];
												$_SESSION['price']=$row3[3];
												echo "<h4><u>Today's Menu</u> : <br><br>". $row3[2] ."     Price :  " . $row3[3]. " 
														<form action=order_menu.php method=POST> 
														<input class=srch type=submit value=order > 
														</form>  </h4>
														<br>";
												}
												else{
													echo "Mess has not updated today's menu yet...<br>";
												}
											}
											else{
												echo "Query Failed !";
											...
											<?php
											$querry4 = "select * from next_menu where mess_id=$row[0] and date='$curr_date'";
											$ret4 = mysql_query($querry4, $conn);
											if($ret4){
												$row4 = mysql_fetch_array($ret4, MYSQL_NUM);
												if($row4){
												echo "<h4><u>Vote For Tomorrows's Menu</u> :      
													<br><br>
													<form action='customer_vote.php?mess_id=$row[0]' method=POST> 
														<input type=radio name='menu' value=votes1>$row4[1]
														<input type=radio name='menu' value=votes2>$row4[2]
														<input type=radio name='menu' value=votes3>$row4[3]<br>  
													<input class=srch id=register type=submit name=submit value=Vote > 
													</form><br>";
												}
														//Print Tommarows_menu here....&&voting
												else{
													echo "Tomorrows menu not available";
												}
											}
											else{
												echo "Mess has not updated tomorrow's menu yet...";
											} 
				...
				<div class="ordering-section-grids">
					<div onclick ="window.location.href = 'customer_profile.php'"  class="col-md-3 ordering-section-grid">
					...
					<div onclick ="window.location.href = 'customer_mess.php'"   class="col-md-3 ordering-section-grid">
						<div class="ordering-section-grid-process wow fadeInRight" data-wow-delay="0.4s"">
							<i class="two"></i><br>
							<i class="two-icon"></i>
							<p>Order  <span>Your Food</span></p>
							<label></label>
						</div>
					</div>
					<div onclick ="window.location.href = 'customer_mess.php'"  class="col-md-3 ordering-section-grid">
						<div class="ordering-section-grid-process wow fadeInRight" data-wow-delay="0.4s"">
							<i class="three"></i><br>
							<i class="four-icon"></i>
							<p>View   <span>Your Mess </span></p>
							<label></label>
						</div>
					</div>
					<div  onclick ="window.location.href = 'customer_bill.php'" class="col-md-3 ordering-section-grid">
						<div class="ordering-section-grid-process wow fadeInRight" data-wow-delay="0.4s"">
							<i class="four"></i><br>
							<i class="one-icon"></i>
							<p>Pay <span>Your Bill </span></p>
						</div>
					</div>
					<div class="clearfix"></div>
				</div>
