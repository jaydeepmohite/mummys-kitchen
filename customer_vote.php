						...
						<?php 
							...
							if(isset($_SESSION['customer_email'])){
								if(isset($_POST['submit'])) {
									...
									$querry2 = "select last_vote_date from customer where customer_id = $customer_id";
									$ret2 = mysql_query($querry2,$conn);
									if($ret2){
										$row2 = mysql_fetch_array($ret2,MYSQL_NUM); 
										if($row2){
											if($row2[0]==$curr_date) {
												echo "<h4>You are Already Voted!!!</h4>";
											}
											else {
												
												$querry = "Update next_menu set $vote = $vote + 1 where mess_id = $mess_id and date='$curr_date'";
												$querry1 = "update customer set last_vote_date = '$curr_date' where customer_id = $customer_id";
												
												$ret  = mysql_query($querry, $conn);
												$ret1 = mysql_query($querry1, $conn);
												if($ret&&$ret1){
													echo "<h4>Thank You for Voting!!!</h4>";
												}
												else
													echo "<h4>Voting Failed...<h4>";
											}
										}
									}
								}
