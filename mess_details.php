...
if( isset( $_GET['name'] ) ) {
	...
	$querry = "SELECT * FROM mess where mess_id = $mess_id ";
	...
	if ($ret) {
		...
		$querry1 = "select * from todays_menu where mess_id=$mess_id and date = '$curr_date'";
		$ret1  = mysql_query($querry1, $conn);
		if($ret1){
			$row1 =  mysql_fetch_array($ret1, MYSQL_NUM);
			if($row1) {
				$_SESSION['menu']=$row1[0];
				$_SESSION['mess']=$mess_id;
				$_SESSION['price']=$row1[3];
				echo "<br><h3><u>Today's Menu</u></h3><br><br><h4>Menu Name - $row1[2]<br><br>Price - $row1[3] RS.</h4><br><form action=single_order.php?amt=$row1[3]&name=$row[0] method=POST><input type=submit value='Order Now' name=submit></form>";
			}
		}
		echo "<br><br><u><h3>Mess Registration</h3></u><br><br><h4>* Deposit Required = $row[6]</h4><br>";
		echo "<form action=customer_mess_register.php?amt=$row[6]&name=$row[0] method=POST><input type=submit value=Register></form>";
	} 
	else {
