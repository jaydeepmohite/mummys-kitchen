...
if( isset( $_SESSION['mess_email'] ) ) {
	...
	$querry = "SELECT mess_name, owner_name, street_name, area_name, city_name, email FROM mess where mess_id = $mess_id ";
	...
	if ($ret) {
		...
		echo	"<br><br><h4><u>Mess Name</u> : {$row[0]} <br><br>". 
				"<u>Owner Name </u>: {$row[1]} <br><br>".
				"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<u>Address </u>: {$row[2]} <br><br>". 
				"{$row[3]} <br><br>". 
				"{$row[4]} <br><br>".
				"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<u>Email </u>: {$row[5]}</h4>";
	} 
	else {
